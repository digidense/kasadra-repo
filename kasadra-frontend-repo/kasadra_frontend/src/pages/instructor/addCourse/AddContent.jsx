...
const AddContent = () => {
  ...
  // -------------------------
  // If coming from AddLesson, pre-select batch and refresh lessons
  // -------------------------
  useEffect(() => {
    ...
    // If refresh flag is set, re-fetch lessons
    if (location.state?.refresh && selectedBatch) {
      dispatch(fetchLessons(Number(selectedBatch)));
      // Optionally, clear the flag
      navigate(location.pathname, { replace: true, state: { batchId: selectedBatch } });
    }
  ...
  // -------------------------
  // Sync lessons from Redux to local state
  // -------------------------
  useEffect(() => {
  ...
  const addNewLesson = () => {
    ...
    // Pass selectedBatch so that AddContent knows which batch to refresh after adding
  ...
  // -------------------------
  // Toggle Activate/Deactivate lesson
  // -------------------------
      ...
      <div className="addcontent-body">
        <div className="addcontent-wrapper">
          <div className="addcontent-header">
            ...
            {/* Batch Dropdown */}
            <div className="addcontent-batch-select" style={{ marginBottom: "18px" }}>
                  ...
                  {lessonsState?.length > 0 ? (
                    lessonsState.map((lesson) => (
                      <tr key={lesson.lesson_id}>
                        <td>{lesson.title}</td>
                        <td>{lesson.course_name
                        }</td>
                        <td>
                          <button
                            onClick={() => handleToggle(lesson.lesson_id)}
                            className={`addcontent-action-btn ${lesson.is_active ? "btn-active" : "btn-inactive"
                              }`}
                          >
